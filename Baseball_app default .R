                                              ...
                                              fluidRow(column(10,offset = 0, align = "center",
                                                      ...
                                                      # column(2, offset = 0, align = "center",
                                                       #       h3("Strike Zone"), hr(style="border-color: black;"),
                                                         #     plotlyOutput("PitchKzone",height = 250, width = 250))
                                              ),
                                              fluidRow(
                                                column(4, offset = 0.5,
...
server <- function(input, output, session) {
  ...
  # * NON-KZONE PITCHING CHARTS *
    ...
    output$SprayChartPlot2 <- renderPlotly({
      ...
      KPlot <- ggplot() + 
        geom_point(data = BatterFirstLast, size = 2, na.rm = T,
                   aes(x = PlateLocSide, y = PlateLocHeight, color = TaggedPitchType))
      KPlot <- Kzone_Chart_Setup(Plot, "pitch")
      KPlot <- Plot %>% ggplotly() 
      subplot(list(Plot,KPlot), nrows = 1)
    ...
    ## Hoverd Kzone graph  for a particular pitch 
     # observeEvent(event_data("plotly_hover", "SprayChartPlot2"),{
      #  hovered_data <- event_data("plotly_hover")
       # if (!is.null(hovered_data)) {
          
     #     output$PitchKzone <- renderPlotly({
      #    filtered_data <- BatterFirstLast()[
       #     BatterFirstLast()$PitcherId == hoveredPoint()$PitcherId &
       #      BatterFirstLast()$Date == hoveredPoint()$Date &
        #      BatterFirstLast()$Time == hoveredPoint()$Time ]
